...
async def vplay(c: Client, m: Message):
    ...
    keyboard = InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(text="‹ اެݪتحكم ›", callback_data="cbmenu"),
                InlineKeyboardButton(text="‹ اެغݪاެق ›", callback_data="cls"),
              ],
              [InlineKeyboardButton(ALIVE_NAME, url=f"t.me/{OWNER_NAME}"),
              InlineKeyboardButton("‹ اެݪسۅٛࢪس ›", url=f"t.me/{UPDATES_CHANNEL}"),
                ],
              [InlineKeyboardButton("‹ اެضف اެݪبۅٛت في مجمۅٛعتك ›", url=f"http://t.me/{BOT_USERNAME}?startgroup=new"),              
            ]
        ]
    )
    if m.sender_chat:
    ...
    try:
        ...
        return await m.reply_text(f"خِطَأ:\n\n{e}")
    ...
    if replied:
        if replied.video or replied.document:
            loser = await replied.reply("𖠧 **جاري تحميل الفيديو...**")
            ...
            if len(m.command) < 2:
                ...
                if search == 0:
                    ...
                    if amort == 0:
                        await loser.edit(f"✗ تم اكتشاف خِطَأ حاول مجددآ\n\n» `{ytlink}`")
                    else:
                        if chat_id in QUEUE:
                            pos = add_to_queue(
                ...
                if veez == 0:
                    ...
                    if chat_id in QUEUE:
                        ...
                        try:
                            ...
                            await loser.delete()
                            await m.reply_text(f"✗ خِطَأ قام احد مشرفي المجموعه بحظر الحساب المساعد برجاء رفع الحظر عنه @{ASSISTANT_NAME} : `{ep}`")
        ...
        if amort == 0:
            ...
            if chat_id in QUEUE:
                ...
                try:
                    ...
                    await loser.delete()
                    await m.reply_text(f"✗ خِطَأ قام احد مشرفي المجموعه بحظر الحساب المساعد برجاء رفع الحظر لكي اعمل @{ASSISTANT_NAME} : `{ep}`")

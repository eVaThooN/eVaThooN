...
async def ytsearch(_, message: Message):
    ...
    try:
        ...
        text = ""
        while i < 5:
            text += f"᥀ **اެݪاެسم:** __{results[i]['title']}__\n"
            text += f"᥀ **اެݪمدة:** `{results[i]['duration']}`\n"
            text += f"᥀ **اެݪمشاެهداެت:** `{results[i]['views']}`\n"
            text += f"᥀ **اެݪقناެة:** {results[i]['channel']}\n"
            text += f"᥀ **اެݪࢪاެبط**: https://www.youtube.com{results[i]['url_suffix']}\n\n"
            i += 1
